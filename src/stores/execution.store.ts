...
export const useExecutionStore = defineStore('execution', () => {
  const state = ref<ExecutionState>({
    status: 'idle', currentNodeId: null, nodeStates: {}, log: [],
  })
  ...
  function reset() {
    ...
    useWorkflowStore().resetAllStates()
  }
  ...
  function onRealtimeUpdate(payload: {
    ...
    for (const [nodeId, nodeState] of Object.entries(ns)) {
      ...
      if (nodeState === 'running') wf.setEdgesAnimatedBySource(nodeId, true)
      if (nodeState === 'success' || nodeState === 'error') wf.setEdgesAnimatedBySource(nodeId, false)
    }
    if (payload.status === 'completed' || payload.status === 'error') isRunning.value = false
  }

  async function run() {
    const wf = useWorkflowStore()
    if (!wf.nodes.find(n => n.type === 'trigger')) return
    reset()
    isRunning.value = true
    try {
      await wf.persist()
      if (wf.workflowId) subscribeToExecution(wf.workflowId, onRealtimeUpdate)

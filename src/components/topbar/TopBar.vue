...
function run() { execStore.run() }
function stop() { execStore.reset() }
...
<template>
  <div class="topbar">
    ...
    <div class="topbar-right">
      <div v-if="isRunning || execStatus === 'completed' || execStatus === 'error'"
        class="status-badge" :class="{ 'status-running': isRunning, 'status-success': execStatus === 'completed', 'status-error': execStatus === 'error' }">
      ...
      <button v-else class="stop-btn" @click="stop">
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><rect x="4" y="4" width="16" height="16" rx="2" fill="currentColor" /></svg>
        <span>Stop</span>
      </button>
...
.brand { display: flex; align-items: center; gap: 7px; color: var(--panel-text); }
.brand-name { font-size: 14px; font-weight: 700; letter-spacing: -0.01em; }
.divider { width: 1px; height: 16px; background: var(--panel-border); }
.workflow-name { font-size: 13px; color: var(--panel-text-muted); }
...
.run-btn { background: rgba(59,130,246,0.15); color: rgb(96,165,250); border: 1px solid rgba(59,130,246,0.3); }
.run-btn:hover { background: rgba(59,130,246,0.25); }
.stop-btn { background: rgba(239,68,68,0.12); color: #f87171; border: 1px solid rgba(239,68,68,0.25); }
.stop-btn:hover { background: rgba(239,68,68,0.2); }

.icon-btn {
  width: 32px; height: 32px; border-radius: 8px; background: none;
  border: 1px solid var(--panel-border); cursor: pointer; display: flex;
  align-items: center; justify-content: center; color: var(--panel-text-muted);
  transition: all 0.15s;
}
.icon-btn:hover { background: rgba(128,128,128,0.1); color: var(--panel-text); }
</style>
